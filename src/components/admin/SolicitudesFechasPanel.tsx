            ...
            {solicitudes.map(p => (
                <div key={p.id} className="bg-white dark:bg-slate-900 border border-orange-200 dark:border-orange-900/50 shadow-sm rounded-xl p-5 transition-colors">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        {editingId === p.id ? (
                            <div className="flex flex-wrap items-center gap-3 bg-gray-50 dark:bg-slate-800/50 p-3 rounded-lg border border-gray-200 dark:border-slate-700">
                                <div className="flex items-center gap-2">
                                    <label className="text-xs font-semibold text-gray-500 dark:text-slate-400">Fecha:</label>
                                    <input
                                        type="date"
                                        value={editFecha}
                                        onChange={e => setEditFecha(e.target.value)}
                                        className="text-sm border border-gray-300 dark:border-slate-600 rounded px-2 py-1 bg-white dark:bg-slate-800 focus:ring-2 focus:ring-[#2B5EA7]/30 dark:text-slate-100"
                                    />
                                </div>
                                <div className="flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-2 ml-auto">
                                    <button onClick={cancelEdit} className="text-xs font-semibold text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 px-3 py-1.5 transition-colors">
                                        Cancelar
                                    </button>
                                    <button
                                        onClick={() => handleAprobar(p.id, editFecha, editTipo as TipoEntrega)}
                                        disabled={processing === p.id}
                                        className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1E3A6E] text-white text-xs font-bold rounded hover:bg-[#2B5EA7] disabled:opacity-50 transition-colors"
                                    >
                            ...
                            <div className="flex flex-wrap items-center gap-4 md:gap-8">
                                <div>
                                    <p className="text-xs text-gray-400 dark:text-slate-500 mb-0.5">Fecha Solicitada:</p>
                                    <p className="font-semibold text-[#1E3A6E] dark:text-slate-200 capitalize">
                                        {format(parseISO(p.fecha_entrega), "EEEE, d 'de' MMMM", { locale: es })}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-400 dark:text-slate-500 mb-0.5">Tipo de Entrega:</p>
                                ...
                                <div className="flex gap-2 w-full md:w-auto mt-2 md:mt-0">
                                    <button
                                        onClick={() => startEdit(p)}
                                        className="flex-1 md:flex-none px-4 py-2 border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 text-sm font-bold rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors"
                                    >
                                        Modificar
                                    </button>
                                    <button
                                        ...
                                        className="flex-1 md:flex-none flex items-center justify-center gap-1.5 px-4 py-2 bg-emerald-600 text-white text-sm font-bold rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors"
                                    >
