        ...
        <div className="bg-white rounded-xl border border-[#E2E5EB] overflow-hidden mb-4">
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-[#E2E5EB]">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-[#1E3A6E] text-sm">{title}</h3>
                    <span className="text-xs text-gray-400">{count} materiales</span>
                </div>
                    ...
                    <thead className="bg-[#F4F6FA] text-gray-500 uppercase tracking-wide text-[10px]">
                        <tr>
                            ...
                            <th className="px-3 py-2.5 text-right font-semibold w-[15%]">
                                Cant. Solicitada
                                <span className="block text-[9px] text-[#2B5EA7] font-normal normal-case">Ingresa aquí</span>
                            </th>
                            ...
                            <th className="px-3 py-2.5 text-right font-semibold w-[15%]">
                                Cant. Kilos
                                <span className="block text-[9px] text-emerald-600 font-normal normal-case">Calculado</span>
                            </th>
                    ...
                    <tbody className="divide-y divide-[#F4F6FA]">
                        {filtered.map(d => {
                            ...
                            return (
                                <tr
                                    key={d.material.id}
                                    className={hasValue ? 'bg-blue-50/40' : 'hover:bg-[#F4F6FA]/60'}
                                >
                                    ...
                                    <td className="px-3 py-1.5 text-right">
                                        {readonly ? (
                                            ...
                                            <input
                                                ...
                                                className="w-24 text-right px-2 py-1 border border-[#2B5EA7]/40 rounded-md font-mono text-xs focus:outline-none focus:ring-2 focus:ring-[#2B5EA7]/40 focus:border-[#2B5EA7] bg-white"
                                                placeholder="0"
                                            />
                                        )}
                                    </td>
                                    ...
                                    <td className="px-3 py-2 text-right text-gray-500 font-mono">{d.material.peso_aproximado ?? '—'}</td>

                                    {/* CANT. KILOS — calculado automáticamente */}
                                    <td className="px-3 py-2 text-right">
                                        <span className={`font-mono font-semibold ${hasValue ? 'text-emerald-700' : 'text-gray-300'}`}>
                                            {cantKilos != null
                                    ...
                                    <td className="px-3 py-2 text-center text-gray-500">{d.material.envase ?? '—'}</td>
                                </tr>
                            )
                        })}
                    </tbody>
